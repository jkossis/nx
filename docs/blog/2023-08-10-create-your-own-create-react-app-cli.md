...
The `addProjectConfiguration` and `generateFiles` functions are from [@nx/devkit](/docs/extending-nx/devkit), a library that contains utility functions for writing plugins for Nx. For the future, see the [complete list of utility functions](/docs/extending-nx/devkit).
...
3\. Our application uses some npm dependencies so add those to the workspace as well with the [addDependenciesToPackageJson](/docs/extending-nx/devkit) function to the end of the export default function in `src/generators/preset/generator.ts`:
